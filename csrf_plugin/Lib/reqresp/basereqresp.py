...
class BaseRequest:
	...
	def __getattr__ (self,name):
		if name=="urlWithoutVariables":
			...
			#return urlunparse((self.schema,self.__host.split(":")[0],self.__path,self.__params,self.__variablesGET.urlEncoded(),''))
...
#		self._headers["Content-Length"]=str(len(self.postdata))
	...
	def getVariableGET(self,key):
		return self.__variablesGET.getVariable(key).value
